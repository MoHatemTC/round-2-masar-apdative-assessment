...
def _parse_llm_grade(text: str | None) -> dict:
    ...
    Falls back to a flagged result if the format can't be parsed — the LLM
    not following instructions exactly is a real, common failure mode."""
    if not text:
    ...
    score = max(0.0, min(5.0, score))  # clamp, same defensive habit as Week 1
